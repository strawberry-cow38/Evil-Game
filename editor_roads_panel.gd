...
func _build_decal_row(index: int, d: Dictionary) -> Control:
	...
	box.add_child(row1)
	_add_field(row1, "off", float(d.get("offset", 0.5)), 0.0, 1.0, 0.05, func(v): _emit_change(index, "offset", v))
	_add_field(row1, "w", float(d.get("width", 0.15)), 0.02, 1.5, 0.05, func(v): _emit_change(index, "width", v))
	var row2 := HBoxContainer.new()
	row2.add_theme_constant_override("separation", 4)
	box.add_child(row2)
	_add_field(row2, "dash", float(d.get("dash_length", 0.0)), 0.0, 20.0, 0.5, func(v): _emit_change(index, "dash_length", v))
	_add_field(row2, "gap", float(d.get("gap_length", 0.0)), 0.0, 20.0, 0.5, func(v): _emit_change(index, "gap_length", v))

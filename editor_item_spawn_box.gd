...
# Editor visual for an item-spawn point. Renders as a small filled cube
# tinted with the owning table's color so the user can see at a glance
# which table feeds which spot. Stores the table id so the editor /
# play-mode bootstrap can look the table up at finalisation.

const SIZE := Vector3(0.6, 0.6, 0.6)
